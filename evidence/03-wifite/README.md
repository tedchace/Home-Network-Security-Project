...
The [later advanced settings screenshot](../02-hardening/04-wpa3-upnp-off-advanced-settings.png) shows WPA3 enabled and UPnP disabled. F03 remains evidence of the earlier pending confirmation, when UPnP appeared enabled. The workflow reports WPA2/WPA3 mixed mode and a stronger PSK. The settings views do not establish WPA3-only mode, per-client negotiated security or resistance to a repeat assessment. They do not demonstrate that the captured old handshake became unusable or that all legacy clients now use WPA3.
